...
public class RxPhoneMgr {
    private static RxPhoneMgr phoneUtil;

    public static RxPhoneMgr getInstance() {
        if (phoneUtil == null) {
            synchronized (RxPhoneMgr.class) {
                if (phoneUtil == null) {
                    phoneUtil = new RxPhoneMgr();
                }
            }
        }
        return phoneUtil;
    ...
    public long getSDFreeSize() {
        if (!isSDCardMount()) return 0; // 先判断SD卡是否挂载
        File path = Environment.getExternalStorageDirectory();
        StatFs sf = new StatFs(path.getPath());
        long blockSize = sf.getBlockSizeLong(); // 替换getBlockSize()
        long freeBlocks = sf.getAvailableBlocksLong(); // 替换getAvailableBlocks()
        // 返回SD卡空闲大小
    ...
    public long getSDAllSize() {
        ...
        StatFs sf = new StatFs(path.getPath());
        long blockSize = sf.getBlockSizeLong(); // 获取单个数据块的大小(Byte)
        long allBlocks = sf.getAvailableBlocksLong(); // 获取所有数据块数
        // 返回SD卡大小
